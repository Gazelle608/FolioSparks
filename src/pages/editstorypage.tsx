...
import { deleteStory, getStoryById, updateStory } from "../api/stories";
...
export function EditStoryPage() {
  ...
  const { desk: storyDesk } = useStoryDesk(storyId ?? null);
  const { desk } = storyDesk
    ? useDeskManagementFallback(storyId, storyDesk)
    : useDeskManagementFallback(storyId, null);
  ...
  return (
    ...
    >
      ...
      {tab === "chapters" && (
        <ChaptersTab story={story} chapters={chapters} polls={polls} />
      )}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={() =>
            navigate(`/studio/story/${story.id}/chapter/${chapters.length + 1}`)}
        >
          New chapter
        </Button>
      </div>
          ...
          {chapters.map((ch) => {
            ...
            return (
              <div key={ch.id} className="p-4 flex items-center gap-4">
                ...
                <Link
                  to={`/studio/story/${story.id}/chapter/${ch.chapter_number}`}
                >
                  <Button
                    variant="ghost"
                    size="sm"
...
function MetaTab({
  ...
  const handleSave = async () => {
    ...
    toast.success("Saved");
    onSaved({ ...story, title, genre, synopsis });
  };
...
// ---------------------------------------------------------------------------
function useDeskManagementFallback(_storyId?: string, desk: any = null) {
  return {
    desk,
    members: [] as any[],
